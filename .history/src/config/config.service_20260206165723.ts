...
export class ConfigService {
  ...
  private async fetchAllProducts(): Promise<ProductDto[]> {
    ...
    // STEP 2: Récupérer les détails complets pour chaque produit (avec links)
    ...
    for (const product of allProducts) {
      try {
        ...
        // Debug: afficher la structure du premier produit détaillé
        if (fetchedCount === 0) {
          console.log('\n📋 [fetchAllProducts] First DETAILED product structure:', JSON.stringify(detailedProduct, null, 2));
          console.log('📋 [fetchAllProducts] First DETAILED product keys:', Object.keys(detailedProduct));
          console.log('📋 [fetchAllProducts] Has links property:', 'links' in detailedProduct);
          console.log('📋 [fetchAllProducts] Links value:', detailedProduct.links);
          console.log('📋 [fetchAllProducts] Links count:', detailedProduct.links?.length ?? 0, '\n');
        }
