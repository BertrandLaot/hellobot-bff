...
import { UrlMappingConfigDto, SiteMappingDto, OnboardingModuleDto } from './dto/url-mapping.dto';
  ...
  async getUrlMappingConfig(): Promise<UrlMappingConfigDto> {
    try {
      ...
      for (const product of products) {
        ...
        // Récupérer le module d'onboarding si présent
        let onboardingModule: OnboardingModuleDto | undefined;
        if (product.onboardingModule?._id) {
          try {
            const modules = await this.backendApiService.get<any[]>(`/products/${product._id}/modules`);
            const onboarding = modules.find(m => m._id === product.onboardingModule._id);
            
            if (onboarding) {
              onboardingModule = {
                _id: onboarding._id,
                name: onboarding.title || onboarding.name,
                image: onboarding.image || onboarding.cover,
                description: onboarding.description,
              };
            }
          } catch (error) {
            // Ignorer les erreurs de récupération des modules
          }
        }
